...
for h in xrange(len(vid)):
    header = "P3\n{0} {1}\n{2}\n".format(640,360,255)
    filename = 'Frames/back_sel{0}.ppm'.format(str(h).zfill(4))
    ...
    for i in xrange(len(vid[h])):
        for j in xrange(len(vid[h][i])):
            ...
            if diff > thresh:
                ...
                fwrite.write("0\n0\n0\n")
                B[i][j] = a*vid[h][i][j] + (1-a)*B[i][j]
os.system("ffmpeg -f image2 -i Frames/back_sel%04d.ppm back_sel.mp4")
os.system("rm Frames/back_sel*")